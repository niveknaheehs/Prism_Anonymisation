...
begin
  ...
  procedure disable_enable_triggers (p_disable_enable  varchar2)
  is
  ...
  begin
      ...
      obfuscation_control.obfus_log('opening get_triggers cursor for '||gp.tgt_prefix,null,null,null,null,null,const_module);  
      ...
      loop
        ...
        exception when others then
           v_code := SQLCODE;
           v_errm := SUBSTR(SQLERRM,1,4000);
           obfuscation_control.obfus_log('Error executing: ' || v_ddl,null,null,null,v_code,v_errm,const_module);  
           raise;
        end;
      end loop;

      obfuscation_control.obfus_log(p_disable_enable||'D '||v_trigger_count||' triggers',null,null,null,null,null,const_module);  
  ...
  begin
     disable_enable_triggers('DISABLE');
  ...
  begin
     disable_enable_triggers('ENABLE');
  end enable_triggers;
