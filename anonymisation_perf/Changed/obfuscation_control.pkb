...
--    -- FAST MASK ENTITITIES
 ...
 procedure load_dd as
  ...
  begin
   ...
   execute immediate 'truncate table dd_part_key_columns';
   execute immediate 'truncate table dd_tab_partitions';
   ...
   insert into dd_tab_partitions (actual_table_owner,table_owner,table_name,partition_name) 
    select table_owner actual_table_owner, replace(table_owner,gp.src_prefix||'_',null) table_owner, table_name,partition_name
      from all_tab_partitions
     where table_owner like gp.src_prefix||'\_%' escape '\'; 

    obfuscation_control.obfus_log(to_char(sql%rowcount) || ' rows inserted into dd_tab_partitions',gp.src_prefix,gp.anon_version,gp.tgt_prefix,null,null,const_module);
  
    insert into dd_part_key_columns (actual_owner,owner,name,object_type,column_name) 
    select owner actual_owner, replace(owner,gp.src_prefix||'_',null) owner,name, object_type,column_name
      from all_part_key_columns
     where owner like gp.src_prefix||'\_%' escape '\'; 

    obfuscation_control.obfus_log(to_char(sql%rowcount) || ' rows inserted into dd_part_key_columns',gp.src_prefix,gp.anon_version,gp.tgt_prefix,null,null,const_module);
